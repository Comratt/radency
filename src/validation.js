    ...
    switch (columnName) {
        ...
        case 'phone':
            return ({
                ...
                valid: `${column}`.replace(/^\+1|^1/, '').length === 10,
            });
        case 'email':
            return ({
                value: column,
                valid: `${column}`.match(/^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/)
            });
        case 'age':
            return ({
                value: column,
        ...
        case 'expiration_date':
            return ({
                value: column,
                valid: checkDate(column)
            });
        case 'license_number':
            return ({
                value: column,
                valid: column.length === 6
            });
        case 'license_states':
            return ({
                value: column.split(',').map((val) => val.trim().length > 2 ? val.trim().slice(0, 2).toUpperCase() : val).join(' | '),
                valid: `${column}`.trim().length >= 2
            });
        case 'has_children':
            return ({
